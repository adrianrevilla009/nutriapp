...
## Addendum — 2026-09-11, by adrianrg1996@gmail.com: Fitbit adapter (mocked, no real credentials) approved

`services/activity-service/CLAUDE.md`'s "Never do this" rule and this plan's §1 MVP scope decision both required a new, separately human-approved plan before any `WearableProviderPort` implementation is built — an implementing agent (`activity-agent`) was tasked with building a Fitbit adapter as part of a broader backlog sweep, correctly refused on its own, and named exactly this addendum as the precondition for proceeding. This addendum is that approval, given directly by the human, not inferred from a task description.

**Approved to proceed**: implement `WearableProviderPort` for **Fitbit only** (OAuth 2.0 Authorization Code flow — token exchange + refresh — and whatever exercise-sync method the port contract requires), following this service's existing resilience-patterns (circuit breaker, retry+backoff, timeout, bulkhead) and mirroring the adapter shape already used elsewhere in this repo for external calls (`food-recognition-service`'s `ClaudeVisionAdapter`, `billing-service`'s Stripe adapter).

**Explicit constraint, unchanged from the original MVP decision**: no real Fitbit developer account or OAuth credentials exist in this environment. The adapter must be built and fully tested against a fake/mocked HTTP client only — never a real network call, in code or in tests, same convention as `ClaudeVisionAdapter`'s "never a live API call in CI" precedent. Wire it behind an explicit config/feature-flag toggle (Unleash, per `.claude/skills/feature-flags/SKILL.md`) so it cannot be silently activated without real credentials being configured later. Update `docs/vendor-risk-register.md`'s Fitbit entry to state the adapter now exists code-wise, structurally tested against mocks, but is unverified against the real API pending a developer account — obtaining that account is a human action, out of scope for this addendum.

Google Fit, Apple Health, and Garmin remain explicitly out of scope and still gated behind their own future human-approved addendum — this approval is Fitbit-only, not a blanket reversal of the "zero implementations" posture for every provider.

Once implemented: persist a test plan addendum, run the full TDD cycle, and report real test/coverage numbers, same as every other fix in this session. No git push/commit — the orchestrating session handles that after independent verification.
