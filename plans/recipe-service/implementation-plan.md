...
## Addendum — 2026-08-30: CLAUDE.md §8 consent surface deferred to frontend

Found during implementation review (`reviewer-agent`): CLAUDE.md §8 requires that "users publishing recipes... are made aware their content/profile becomes visible to other users; this is a distinct consent surface from the general product ToS." Neither this plan nor the built `PublishRecipeCommand`/`POST /api/v1/recipes/{recipe_id}/publish` route addresses this — no consent flag exists on the publish request, and the original plan's §1 out-of-scope list didn't call this out explicitly the way it did for other deferred items (recipe images, ratings/reviews).

**Decision (this addendum):** this consent surface is a frontend/UX concern — a confirmation step shown to the user before they submit a publish request (e.g. "your recipe will become visible to other NutriApp users" with an explicit acknowledgment), not a backend data field this service needs to store or enforce today. `recipe-service`'s publish endpoint has no way to distinguish "user was shown and accepted the notice" from "user wasn't," and adding an unenforceable boolean flag to the request body would be worse than no flag at all (implies a guarantee the backend can't actually keep). This is deferred to `docs/frontend-architecture.md` (not yet written for this specific flow) as a UI-layer requirement that must exist before `recipe-service`'s publish endpoint is exposed to real users — tracked here explicitly so it isn't silently forgotten, per `reviewer-agent`'s finding, rather than assumed out of scope without a written decision.
