"""SendPasswordResetEmailHandler -- test-plan section 1 (same cases as
...
from domain.ports.token_reveal_port import TokenRevealNotFoundError
...
def _command(event_id: uuid.UUID | None = None) -> SendPasswordResetEmailCommand:
    return SendPasswordResetEmailCommand(
        event_id=event_id or uuid.uuid4(),
        user_id=uuid.uuid4(),
        email="user@example.com",
        reset_token_reference_id=str(uuid.uuid4()),
...
async def test_reveal_failure_never_sends_and_logs_failed_and_raises():
    handler, token_reveal, email_provider, delivery_log, processed, _suppression = _build_handler()
    token_reveal.error_to_raise = TokenRevealNotFoundError("unknown reference id")
    command = _command()
