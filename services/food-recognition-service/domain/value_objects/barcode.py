...
Own independent copy of catalog-service's identical value object, per
CLAUDE.md section 2.5 ("no shared schemas/code across service
boundaries" -- each service's own copy of a shared *pattern*, not a
cross-service import). A `Barcode` is either a valid, check-digit-verified
GTIN-8/12/13/14, or absent (`None`) -- never an unvalidated raw string
masquerading as a barcode.
"""
