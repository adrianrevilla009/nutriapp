...
name (`food-recognition`, dropping the `-service` suffix), topic exchange
...
EXCHANGE_NAME = "food-recognition.events"

_ROUTING_KEYS: dict[str, str] = {
    "FoodPhotoAnalyzed": "food-recognition.photo.analyzed",
}


def routing_key_for(event_type: str) -> str:
    return _ROUTING_KEYS.get(event_type, f"food-recognition.event.{event_type.lower()}")
