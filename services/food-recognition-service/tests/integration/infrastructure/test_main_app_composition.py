...
def test_create_app_registers_all_expected_routes():
    ...
    assert "/api/v1/recognition/photos/analyze" in paths
    assert "/api/v1/recognition/barcodes/decode" in paths
...
async def test_lifespan_starts_and_stops_the_real_container(db_engine, postgres_async_url):
    with RabbitMqContainer("rabbitmq:3.13-management-alpine") as rabbitmq_c:
        ...
        monkeypatch.setenv("FOOD_RECOGNITION_SERVICE_DATABASE_URL", postgres_async_url)
        monkeypatch.setenv("FOOD_RECOGNITION_SERVICE_RABBITMQ_URL", rabbitmq_url)
        monkeypatch.setenv("FOOD_RECOGNITION_SERVICE_ANTHROPIC_API_KEY", "test-key")
