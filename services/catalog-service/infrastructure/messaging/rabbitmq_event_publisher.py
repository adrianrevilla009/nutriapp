...
topic exchange per producing service (`catalog.events`).
"""
...
_ROUTING_KEYS: dict[str, str] = {
    "ProductCatalogued": "catalog.product.catalogued",
    "ProductUpdated": "catalog.product.updated",
}


def routing_key_for(event_type: str) -> str:
    return _ROUTING_KEYS.get(event_type, f"catalog.product.{event_type.lower()}")
