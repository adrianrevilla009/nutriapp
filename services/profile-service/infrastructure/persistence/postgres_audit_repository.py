...
`audit_records` (see migrations/versions/0003_create_audit_records_table.py
and observability-audit SKILL.md). Commits independently of the caller's
main transaction so an audit record survives even if the triggering
operation is subsequently rolled back by the caller.
"""
