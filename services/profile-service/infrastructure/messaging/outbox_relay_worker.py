"""OutboxRelayWorker -- the separate relay process/task described in
messaging-conventions SKILL.md's Outbox Pattern section.

Each event is published and marked-published individually so a crash
mid-relay never loses an event (stays unpublished, retried next poll) and
never republishes an already-published row.
"""
