...
async def test_redis_rate_limiter__different_keys_tracked_independently(redis_client):
    limiter = RedisRateLimiter(redis_client)
    await limiter.check_and_increment("caller-a:user-1", limit=1, window_seconds=60)
    await limiter.check_and_increment("caller-a:user-2", limit=1, window_seconds=60)
    with pytest.raises(RateLimitExceededError):
        await limiter.check_and_increment("caller-a:user-1", limit=1, window_seconds=60)
