...
back to identity-service on every request. Reuses
`shared_contracts.auth.dependencies` per established precedent
(`food-recognition-service`, `notification-service`, `bff-service`); this
module only wires this service's own `Container` into it.
"""
