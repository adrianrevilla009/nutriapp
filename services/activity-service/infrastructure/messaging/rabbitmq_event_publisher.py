...
topic exchange per producing service (`activity.events`).
"""
...
EXCHANGE_NAME = "activity.events"

_ROUTING_KEYS: dict[str, str] = {
    "ExerciseLogged": "activity.exercise.logged",
}


def routing_key_for(event_type: str) -> str:
    return _ROUTING_KEYS.get(event_type, f"activity.exercise.{event_type.lower()}")
