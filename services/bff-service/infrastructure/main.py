...
(messaging-conventions SKILL.md is N/A -- this service publishes/consumes
no events, implementation plan section 2)."""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("bff_service_started")
    try:
        yield
    finally:
        await container.shutdown()
        logger.info("bff_service_stopped")
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="bff-service",
        version="0.1.0",
        description="NutriApp bff-service: frontend response aggregation only -- "
        "orchestration, never business logic (ADR-0008).",
        lifespan=lifespan,
    )
