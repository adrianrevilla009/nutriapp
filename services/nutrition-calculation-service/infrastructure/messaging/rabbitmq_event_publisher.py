...
Naming convention (messaging-conventions SKILL.md): short producing-service
name (`nutrition-calculation`, dropping the `-service` suffix), topic
exchange `nutrition-calculation.events`.
"""
...
_ROUTING_KEYS: dict[str, str] = {
    "NutritionTargetUpdated": "nutrition-calculation.target.updated",
    "NutritionValueRecomputed": "nutrition-calculation.total.recomputed",
}


def routing_key_for(event_type: str) -> str:
    return _ROUTING_KEYS.get(event_type, f"nutrition-calculation.event.{event_type.lower()}")
