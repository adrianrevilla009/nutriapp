...
def test_create_app_registers_all_expected_routes():
    ...
    assert "/api/v1/nutrition/target" in paths
    assert "/api/v1/nutrition/target/history" in paths
    assert "/api/v1/nutrition/totals/{total_date}" in paths
...
async def test_lifespan_starts_and_stops_the_real_container(
    db_engine, postgres_async_url, redis_url
):
    import pytest
    ...
    with RabbitMqContainer("rabbitmq:3.13-management-alpine") as rabbitmq_c:
        ...
        monkeypatch.setenv("NUTRITION_CALCULATION_SERVICE_DATABASE_URL", postgres_async_url)
        monkeypatch.setenv("NUTRITION_CALCULATION_SERVICE_REDIS_URL", redis_url)
        monkeypatch.setenv("NUTRITION_CALCULATION_SERVICE_RABBITMQ_URL", rabbitmq_url)
