...
Idempotency dedup for diary_event_projector_consumer (messaging-
conventions SKILL.md). A processed-event record older than
PROCESSED_EVENT_TTL is treated as eligible for reprocessing -- long enough
to exceed realistic RabbitMQ redelivery windows, short enough that this
table does not grow unbounded (a periodic cleanup job deleting expired
rows is a documented follow-up, not implemented here).
"""
