...
async def test_outbox__enqueue_and_row_insert_are_atomic_with_the_triggering_write(
    session, db_engine
):
    repo = PostgresOutboxRepository(session)
    event = make_event()
    await repo.enqueue(event)
    # Simulate a failure before commit: rollback instead.
    ...
    async with AsyncSession(db_engine) as verify_session:
        ...
        assert result.scalar_one_or_none() is None  # never committed, nothing lost/leaked


async def test_outbox__fetch_unpublished_then_mark_published__does_not_republish(session):
    repo = PostgresOutboxRepository(session)
    event = make_event()
    await repo.enqueue(event)
...
async def test_outbox__simulated_crash_mid_relay_does_not_lose_the_event(session, db_engine):
    repo = PostgresOutboxRepository(session)
    event = make_event()
    await repo.enqueue(event)
    await session.commit()

    # Simulate a relay worker crashing after publish but before mark_published:
    # the event is never marked published, so the next poll picks it up again.
