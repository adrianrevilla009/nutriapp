"""FastAPI dependency providers: request-scoped DB session, inbound
correlation id, and the authenticated caller's user id -- structurally
identical to recipe-service's/social-service's own `dependencies.py`
(ADR-0022, docs/authorization-model.md section 2)."""
...
def get_container(request: Request) -> Container:
    # Starlette's `Request.app.state` is a dynamic-attribute `State` object
    # typed as returning `Any` for any attribute access -- mypy can't see
    # that `main.py`'s lifespan always sets `.container` to a real
    # `Container` before any request is served. Genuine false positive,
