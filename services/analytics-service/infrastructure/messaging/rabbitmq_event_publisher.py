"""RabbitMqEventPublisher -- analytics-service's `EventPublisherPort`
adapter, the far end of the outbox relay. Publishes to this service's own
topic exchange (`analytics.events`), routing key per the repo-wide
`{producing_service}.{aggregate}.{event_type_snake_case}` convention
(messaging-conventions SKILL.md)."""
...
_ROUTING_KEYS: dict[str, str] = {
    "NutrientDeficiencyDetected": "analytics.anomaly.nutrient_deficiency_detected",
}
_FALLBACK_ROUTING_KEY_TEMPLATE = "analytics.anomaly.{event_type}"
