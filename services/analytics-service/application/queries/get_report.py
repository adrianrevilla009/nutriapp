...
cache miss (`application.entitlement_check.is_user_entitled`), rejected
BEFORE any report-data repository read and before any `export_audit_log`
write (cheapest-check-first, mirrors `recipe-service`'s/`social-service`'s
precedent).

Every successful export is recorded in `export_audit_log` exactly once
per call -- not deduplicated like event-consumption idempotency, every
export is its own audit fact (test-plan section 2's explicit rule).
