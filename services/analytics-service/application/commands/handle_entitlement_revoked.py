"""Consumes billing-service's `EntitlementRevoked` (v1) -- only flips the
cached flag, never touches `daily_log_summary`/`micronutrient_window`/
`weight_trend`/`anomaly_alerts` (non-destructive, structural guard: this
handler's constructor has no reference to any of those repositories at
all, mirroring recipe-service's/social-service's identical handler)."""
