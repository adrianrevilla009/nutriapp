...
redelivering the same `FoodEntryLogged` event twice results in exactly
one contribution applied (idempotency, test-plan section 3); a message
...
async def _poll_daily_totals(session_factory, user_id, on_date, *, attempts=20, interval=0.25):
    for _ in range(attempts):
        async with session_factory() as session:
            rows = await PostgresDailyLogSummaryRepository(session).list_window(
...
async def test_redelivering_the_same_food_entry_logged_event_applies_exactly_once(
    ...
    try:
        ...
        row = await _poll_daily_totals(session_factory, user_id, datetime.now(timezone.utc).date())
        await asyncio.sleep(0.5)  # let a possible second delivery finish processing

        assert row is not None
        assert row.calories_kcal == 200.0  # 2.0 * 100.0, applied exactly once
    finally:
