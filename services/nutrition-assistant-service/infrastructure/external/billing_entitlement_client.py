...
GET /internal/v1/billing/entitlements/{user_id}, sending the shared
X-Internal-Service-Credential (same single-shared-credential mechanism as
recipe-service's/social-service's/analytics-service's other
internal-endpoint consumers).

Own, DEDICATED purgatory circuit breaker (billing_entitlement_check) --
never shared with claude_conversation. Used ONLY on an
EntitlementCacheRepositoryPort cache miss -- the caller
(application.entitlement_check.is_user_entitled) decides that, not this
adapter.
