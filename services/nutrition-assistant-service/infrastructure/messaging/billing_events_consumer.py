...
saga's fan-out. nutrition-assistant-service is the FOURTH real consumer
(after recipe-service, social-service, analytics-service) -- see the
implementation plan addendum, 2026-09-08. Idempotent by `event_id`. Any
other billing event type is acknowledged and ignored."""
...
QUEUE_NAME = "nutrition-assistant-service.billing_entitlement_events"
DLQ_NAME = "nutrition-assistant-service.billing_entitlement_events.dlq"
...
async def dispatch_billing_event(
    ...
    processed_events = PostgresEntitlementEventsRepository(session)
