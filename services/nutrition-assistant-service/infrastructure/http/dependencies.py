"""FastAPI dependency providers -- structurally identical to
analytics-service's/recipe-service's/social-service's own dependencies.py
(ADR-0022, docs/authorization-model.md section 2). `get_authenticated_user_id`
is the ONE source of user identity used anywhere in this service's HTTP
layer -- never taken from a request body/query field, which is what makes
cross-user isolation structurally enforceable (implementation plan
