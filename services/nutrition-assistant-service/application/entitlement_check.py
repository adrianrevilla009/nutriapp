"""The one entitlement-gating check AnswerChatQueryHandler funnels
through -- the chat feature is Pro-gated (implementation plan section 1
acceptance criterion 2, product-requirements.md feature 19).

Strategy: cache-first, falling back to the synchronous
EntitlementCheckPort ONLY on a genuine cache miss (entitlement_cache.get()
returns None, not False). The fallback result is deliberately never
written back into the cache -- this function holds no reference to any
write method on EntitlementCacheRepositoryPort at all, the same
structural guarantee recipe-service's/social-service's/analytics-service's
identical helper provides.
