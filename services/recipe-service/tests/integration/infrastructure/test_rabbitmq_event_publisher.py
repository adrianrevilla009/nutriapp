...
async def test_publishes_recipe_created_to_correct_exchange_and_routing_key(amqp_url):
    ...
    try:
        ...
        routing_key = routing_key_for("RecipeCreated")
        assert routing_key == "recipe.recipe.created"
        await queue.bind(exchange, routing_key=routing_key)

        recipe_id = uuid.uuid4()
        user_id = uuid.uuid4()
        event = build_recipe_created_event(
            recipe_id=recipe_id, user_id=user_id, correlation_id="corr-1"
        )
        await publisher.publish(event)

        received = await asyncio.wait_for(queue.get(timeout=5, fail=True), timeout=6)
        body = json.loads(received.body)
        assert body["event_type"] == "RecipeCreated"
        assert body["payload"]["recipe_id"] == str(recipe_id)
        await received.ack()
    finally:
        await connection.close()


def test_unknown_event_type_falls_back_to_generic_routing_key():
    assert routing_key_for("SomeFutureRecipeEvent") == "recipe.recipe.somefuturerecipeevent"
