...
route's `Authorization: Bearer <token>` header carries a RS256 JWT issued
by identity-service, verified LOCALLY via
...
miss/expiry. Mirrors billing-service's/profile-service's identical
dependency, per implementation plan section 4's explicit instruction to
reuse the centralized JWT auth dependency for every public route.
"""
