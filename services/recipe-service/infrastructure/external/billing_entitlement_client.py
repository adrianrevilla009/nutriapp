...
section 1.7/3), sending the shared `X-Internal-Service-Credential` this
service is provisioned with (same single-shared-credential mechanism as
identity-service/catalog-service's other internal-endpoint consumers).

Own, DEDICATED `purgatory` circuit breaker (`billing_entitlement_check`)
-- never shared with `catalog_product_lookup` (implementation plan
section 7). Used ONLY on an `EntitlementCacheRepositoryPort` cache miss --
the caller (`is_user_entitled`) decides that, not this adapter.
