"""OutboxRelayWorker -- mirrors services/recipe-service's/billing-service's
adapter exactly (messaging-conventions SKILL.md's Outbox Pattern relay is
a repo-wide convention). Each event is published and marked-published
individually so a crash mid-relay never loses an event and never
