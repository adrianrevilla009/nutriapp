...
distributed-transactions.md) -- social-service is the SECOND real
consumer of this event (recipe-service was the first). Idempotent by
`event_id`: the idempotency check short-circuits BEFORE any cache write
(test-plan section 1's explicit "cache-repository's write method called
exactly once total across both invocations" assertion). Mirrors
recipe-service's identical handler exactly."""
