"""Shared entitlement-check helper -- used by `FollowUserHandler`,
`UnfollowUserHandler`, and `GetFeedHandler` (implementation plan section
1.2): cache-first, falling back to the synchronous `EntitlementCheckPort`
ONLY on a genuine cache miss (`entitlement_cache.get()` returns `None`).
The fallback result is deliberately NEVER written back into the cache --
this function has no reference to any write method on
...
section 1's explicit assertion). Mirrors `recipe-service`'s
`application/entitlement_check.py` verbatim.
