...
| `nutrition-assistant-service` | Yes | Yes — 149 tests passing (95 unit+contract + 54 integration via testcontainers Postgres/RabbitMQ/Qdrant; domain 98%, application 100%, infrastructure 88% coverage) | No — Terraform written (`nutrition-assistant-service.tf`, new `modules/qdrant/`) and `validate`-clean, `apply` not yet run | No | 2026-09-07 — first implementation, last of the 14 bounded contexts. Conventional persistence/event-driven CRUD (ADR-0002), consuming `diary-service`/`nutrition-calculation-service`/`analytics-service` events into structured history projections — first service to use Qdrant (self-hosted, shared platform instance) and second to call an LLM (Claude Haiku 4.5, cheapest tier, same precedent as `food-recognition-service`). Embeddings via self-hosted `sentence-transformers/all-MiniLM-L6-v2` (Apache-2.0, no new vendor/DPA). `POST /api/v1/chat` Pro-gated, cache-first entitlement check against `billing-service`. Both release-blocking test categories (cross-user isolation, professional-advice-boundary disclaimer) pass, structurally enforced against a fake LLM response to prove it's not model-trusted. **Known gaps**: `entitlement_cache` has no live writer yet (every request falls through to the synchronous check — safe, no caching benefit); rule-based health-topic classifier has real precision/recall limits, flagged for `security-agent` review before staging/prod, same posture as `analytics-service`'s deficiency threshold; knowledge-base seed content (9 files) explicitly marked `STATUS: DRAFT`, pending human/professional review before production use. Completes all 14 backend bounded contexts |
| `frontend` | Yes | Yes — journeys 1+2 ("register → log a food item from catalog search → see macro/micro totals" and "upload a food photo → AI detects the item → logged with computed nutrients"). Journey 1: 100 tests, 93.71%/80.45% stmt/branch coverage, real Playwright E2E 2/2 passing. Journey 2 (2026-09-10): 165/165 unit+integration tests passing, 96.97%/90.17% stmt/branch coverage — the AI candidate's name routes through the existing catalog-search flow to a real, confirmed product (food-recognition-service returns no nutrient data; diary-service never fabricates macros, one shared code path proven via a parameterized test), logged as `source_type: "ai_detected"` with `X-Correlation-Id` set to the analysis ID for traceability (per `architecture-agent` review — first-ever real use of `ai_detected`, reviewed and ALIGNED WITH NOTES). Verified live against the real stack: real login, real multipart photo upload, real `FoodEntryLogged` row with the correlation ID confirmed byte-for-byte in the event store, real dashboard totals (macros correct, micronutrients correctly `"unavailable"` — the accepted, predicted degrade path for this source type). First-ever frontend code in this repo (Next.js 15, TypeScript strict, TanStack Query, Zod, `next-intl`). All browser-facing calls proxy through this app's own Route Handlers (no direct browser→backend calls, no Kong/CORS dependency yet). Everything beyond journeys 1-2 (barcode logging, Pro features, chat, recipes, social, wearables) is explicitly deferred | No — Terraform written (`frontend.tf`) and `validate`-clean, Helm chart `lint`/`template`-clean, `apply`/`install` not yet run | No | 2026-09-10 — journey 2 landed. **Known gaps**: journey 2's Playwright E2E spec (`photo-log-dashboard.spec.ts`) is structurally verified (typecheck/lint/`--list` clean) but never executed by an actual browser in this environment (missing `libnspr4.so`, no sudo available) — a real run is still needed before this is considered fully verified; same for a genuine keyboard-only/screen-reader manual pass (automated axe-core + structural HTML checks stand in for it here). `food-recognition-service`'s ingress `NetworkPolicy` doesn't yet allow `frontend`'s pod selector (same known gap pattern as the other 4 downstream services from journey 1). A real backend design question is tracked, not resolved: should `ai_detected`-sourced-but-catalog-matched entries instead be `source_type: "catalog_product"` + a separate `ai_analysis_id` field, to restore micronutrient-panel eligibility (currently permanently `"unavailable"` for this cohort)? Deferred as a distinct future plan per `architecture-agent`'s review, not blocking. E2E spec still not wired into `frontend-ci.yml` (needs Docker-in-Docker CI runner support, same journey-1 gap). Token-storage mechanism reviewed and cleared 2026-09-08 (`security-agent`, empirically correct). No design system/visual identity beyond a minimal accessible default |
