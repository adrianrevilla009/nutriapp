...
| `frontend` | Yes | Yes — journey 1 only ("register → log a food item from catalog search → see macro/micro totals"), 100 tests passing (58 unit + 42 integration incl. axe-core on every screen; 93.71% statement / 80.45% branch coverage) + a real Playwright E2E spec run against a live `docker-compose` stack, 2/2 passing. First-ever frontend code in this repo (Next.js 15, TypeScript strict, TanStack Query, Zod, `next-intl`). All browser-facing calls proxy through this app's own Route Handlers (no direct browser→backend calls, no Kong/CORS dependency yet). Everything beyond journey 1 (photo/barcode logging, Pro features, chat, recipes, social, wearables) is explicitly deferred | No — Terraform written (`frontend.tf`) and `validate`-clean, Helm chart `lint`/`template`-clean, `apply`/`install` not yet run | No | 2026-09-08 — first implementation. Found and fixed 7 real bugs via genuine E2E testing against a live stack (not just mocked), several only catchable that way: a `Secure`-cookie flag silently breaking login persistence over local HTTP; a schema/contract layering bug where the browser client validated the proxy's response against the full backend schema (which the proxy deliberately strips a field from), silently breaking every real login behind the UI's intentionally-generic error copy; a shared-rate-limit-bucket risk from proxying login/register without forwarding `X-Forwarded-For`. **Known gaps**: E2E spec not yet wired into `frontend-ci.yml` (needs Docker-in-Docker CI runner support); token-storage mechanism (in-memory access token + httpOnly refresh cookie) empirically works but not yet `security-agent`-reviewed; no design system/visual identity beyond a minimal accessible default (no `.claude/skills/frontend-design` exists yet). **Also surfaced, not fixed here (pre-existing, blocks any real deploy)**: `diary-service`/`nutrition-calculation-service`/`bff-service`'s Dockerfiles have a broken venv shebang between build and runtime stages — every container crashes on start via `docker compose build`; `identity-service`'s migration needs a Postgres role docker-compose never provisions; `diary-service`'s RabbitMQ/database env vars are missing from `docker-compose.yml`/`.env.example`. Flagged for `devops-agent`/the respective owning agents |
