...
| `notification-service` | Yes | Yes — 166 tests passing (domain 100%, application ~99%, infrastructure ~97% coverage) | No — Terraform written (`notification-service.tf`) and `plan`-validated, `apply` not yet run | No | 2026-08-31 — PR (this change) merged: real `UserFollowed` (social-service) consumer added — `new_follower` push category, quiet-hours-respecting via a new one-shot `pending_push_dispatch` table + scan worker (kept separate from `reminder_schedule`'s periodic shape, since a one-shot trigger has no "next occurrence" to retry against), ADR-0011 addendum recording the required sign-off. First of the two coordinated PRs behind `social-service`'s own initiative. Builds on 2026-08-28 PR #14's base (CQRS read-side-only, transactional email, periodic push reminders) |
